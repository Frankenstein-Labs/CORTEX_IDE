// FILE: vite.config.ts
// Purpose: Builds the Cortex web client and controls diagnostic source maps.
...
const sourcemapEnv = process.env.CORTEX_WEB_SOURCEMAP?.trim().toLowerCase();
...
function centralIconPrunePlugin(): Plugin {
  ...
  return {
    name: "cortex-central-icon-prune",
    apply: "build",
...
export default defineConfig({
  plugins: [
    ...
    babel({
      ...
      // This is causing the vendored contracts/ and shared/ directories to fail to parse.
