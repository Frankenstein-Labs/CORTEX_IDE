...
import { defaultTerminalTitleForCliKind } from "@cortex/shared/terminalThreads";
import { isThreadDetailEventFor } from "@cortex/shared/threadDetailEvents";
import {
  Outlet,
...
function TransportCompatibilityView({ issue }: { issue: WsCompatibilityError }) {
  const title =
    issue.action === "update-client"
      ? "This Cortex client needs an update."
      : issue.action === "update-server"
        ? "The Cortex server needs an update."
        : "Cortex needs to reconnect with a matching build.";
  const guidance =
    issue.action === "update-client"
      ? "Update or reload this client, then reconnect."
      : issue.action === "update-server"
        ? "Update or restart the server, then reload this client."
        : "Reload the app. If this repeats, restart Cortex so the client and server use matching builds.";
