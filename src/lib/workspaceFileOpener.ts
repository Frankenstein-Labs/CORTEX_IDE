...
import { isSupportedLocalPreviewFilePath } from "@cortex/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
...
const CORTEX_PUBLIC_ASSET_PATH_PREFIXES = [
  "/central-icons-reversed/",
  "/central-icons-fill/",
] as const;
const CORTEX_WEB_PUBLIC_WORKSPACE_DIR = "public";

function resolveCortexPublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
  const normalizedPath = path.replace(/\\/g, "/");
  if (!CORTEX_PUBLIC_ASSET_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
    return null;
  }
  const relativePath = `${CORTEX_WEB_PUBLIC_WORKSPACE_DIR}${normalizedPath}`;
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveCortexPublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
